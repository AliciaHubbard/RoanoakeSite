...
If (CStr(Request("MM_update")) = "form1") Then
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "UPDATE tbl_attractions SET a_name = ?, a_description = ?, a_admission = ?, type = ?, approved = ? WHERE a_name = ?" 
    ...
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param6", 200, 1, 255, Request.Form("MM_recordId")) ' adVarChar
    MM_editCmd.Execute
...
If (Request.QueryString("a_name") <> "") Then 
  Recordset1__MMColParam = Request.QueryString("a_name")
End If
%>
<%
Dim Recordset1
...
Recordset1_cmd.CommandText = "SELECT * FROM tbl_attractions WHERE a_name = ?" 
...
<form method="POST" action="<%=MM_editAction%>" name="form1">
  <table align="center">
    <tr valign="baseline">
      ...
      <td><input name="a_name" type="text" value="<%=(Recordset1.Fields.Item("a_name").Value)%>" size="45" ></td>
    </tr>
    <tr valign="baseline">
      <td nowrap align="right">Description:</td>
      <td><textarea name="a_description" id="a_description" cols="45" rows="5"><%=(Recordset1.Fields.Item("a_description").Value)%></textarea></td>
    </tr>
    <tr valign="baseline">
      <td nowrap align="right">Admission:</td>
      <td><input type="text" name="a_admission" value="<%=(Recordset1.Fields.Item("a_admission").Value)%>" size="32"></td>
    </tr>
    <tr valign="baseline">
...
%>
        <option value="<%=(rs_type.Fields.Item("type").Value)%>" <%If (Not isNull((Recordset1.Fields.Item("type").Value))) Then If (CStr(rs_type.Fields.Item("type").Value) = CStr((Recordset1.Fields.Item("type").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%> ><%=(rs_type.Fields.Item("type").Value)%></option>
        <%
  rs_type.MoveNext()
Wend
...
%>
    ...
    <tr valign="baseline">
      <td nowrap align="right">Approved (1 = yes, 0 = no):</td>
      <td><input type="text" name="approved" value="<%=(Recordset1.Fields.Item("approved").Value)%>" size="32"></td>
    </tr>
    <tr valign="baseline">
  ...
  <input type="hidden" name="MM_recordId" value="<%= Recordset1.Fields.Item("a_name").Value %>">
</form>
<p>&nbsp;</p>
<p></p>
<!-- InstanceEndEditable -->
